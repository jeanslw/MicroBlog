...
def test_banner_list_shows_items(login_admin, db):
    ...
    db.session.add(Banner(
        img_path="/static/banner/x.jpg", title="T1",
        link_url="https://example.com", sort=1,
...
def test_banner_add_success(login_admin, db, app):
    ...
    rv = login_admin.post("/banner/add", data={
...
def test_banner_edit_metadata(login_admin, db):
    """仅编辑文字字段（不换图）应成功"""
    b = Banner(img_path="/static/banner/old.jpg", title="旧",
               create_time="2026-01-01 00:00:00")
    db.session.add(b)
    db.session.commit()

    rv = login_admin.post(f"/banner/edit/{b.id}", data={
        "banner_img": (io.BytesIO(b""), ""),
        "title": "新标题",
...
def test_banner_edit_replace_image(login_admin, db):
    """更换图片应更新 img_path"""
    b = Banner(img_path="/static/banner/old.jpg", title="x",
               create_time="2026-01-01 00:00:00")
    db.session.add(b)
    db.session.commit()
    old_path = b.img_path
...
def test_banner_edit_not_found(login_admin):
    """编辑不存在的 banner 应重定向"""
    rv = login_admin.post("/banner/edit/99999", data={
        "title": "x", "link_url": "", "desc_text": "", "sort_num": 0,
    }, content_type="multipart/form-data", follow_redirects=False)
    assert rv.status_code == 302
...
def test_banner_del_success(login_admin, db):
    """删除 banner 应从数据库移除"""
    b = Banner(img_path="/static/banner/del.jpg", title="待删",
               create_time="2026-01-01 00:00:00")
    db.session.add(b)
    db.session.commit()
    bid = b.id
