...
def test_site_favicon_default(client):
    """默认 favicon 应指向 /static/favicon.ico"""
    rv = client.get("/")
    assert rv.status_code == 200
    assert b'href="/static/favicon.ico"' in rv.data
...
def test_site_favicon_custom(client, db):
    ...
    rv = client.get("/")
    assert rv.status_code == 200
    assert b"uploads/custom.ico" in rv.data
