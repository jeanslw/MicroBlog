...
def test_article_new_publish(login_admin, db, category):
    """发布公开文章后应出现在首页"""
    rv = login_admin.post("/article/new", data={
        "title": "发布测试文章",
        "content": "<p>正文内容</p>",
        "status": "publish",
...
def test_article_new_requires_login(client):
    """未登录不能新建文章"""
    rv = client.post("/article/new", data={
        "title": "x", "content": "y", "status": "publish", "category_id": 0,
    }, follow_redirects=False)
    assert rv.status_code == 302
...
def test_article_edit_save(login_admin, article, category, db):
    """编辑后内容应更新"""
    rv = login_admin.post(f"/article/edit/{article.id}", data={
        "title": "已修改标题",
        "content": "<p>新内容</p>",
        "status": "publish",
        "category_id": category.id,
...
def test_add_category_success(login_admin, db):
    """新增栏目应成功"""
    rv = login_admin.post("/category/add", data={
        "cat_name": "新栏目",
        "tag_text": "tag",
    }, follow_redirects=False)
    assert rv.status_code == 302
...
def test_add_category_requires_login(client):
    """未登录不能新增栏目"""
    rv = client.post("/category/add", data={
        "cat_name": "x", "tag_text": "",
    }, follow_redirects=False)
    assert rv.status_code == 302
    assert "/login" in rv.headers.get("Location", "")
